...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, bds, render_poses = load_real_data(args.datadir, factor=args.factor,
                                                          recenter=True, spherify=args.spherify)
        hwf = poses[0, :3, -1]
        poses = poses[:, :3, :4]
        print('Loaded real data', images.shape, render_poses.shape, hwf, args.datadir)
        i_test = np.arange(images.shape[0])[::args.llffhold]  # 自动划分测试集
        i_val = i_test
        i_train = np.array([i for i in np.arange(int(images.shape[0])) if
                            (i not in i_test and i not in i_val)])
